...
CHIP_ERROR AppTask::Init()
{
	...
	LOG_DBG("App has been initialised");
...
CHIP_ERROR AppTask::StartApp()
{
	...
	LOG_INF("Entering loop to track queue");
	while (true) {
		LOG_DBG("Current event: %" PRId8, uint8_t(event.Type));
		k_msgq_get(&sAppEventQueue, &event, K_FOREVER);
		DispatchEvent(event);
	}

	LOG_DBG("Exited loop to track queue");
	return CHIP_NO_ERROR;
}
...
{
	int putResult = k_msgq_put(&sAppEventQueue, &event, K_NO_WAIT);
	LOG_INF("Event put result: %" PRId8, putResult);
	if (putResult != 0) {
		LOG_INF("Failed to post event to app task event queue");
	}

	LOG_DBG("Event in queue: %" PRId8, uint8_t(event.Type));
}

void AppTask::DispatchEvent(const AppEvent &event)
{
	LOG_INF("Event type: %" PRId8, uint8_t(event.Type));
	if (event.Handler) {
		event.Handler(event);
	} else {
